...
public class OptimalParallelBenchmarkAnalyzer {
  ...
  // Basic fields
  @Setter private String benchRepoPath; // The path of the benchmark repository

  @Setter @Getter
  private PropertySet
      properties; // The set of properties against which the project should be analyzed
  ...
  /**
   * This method is used in order to pass the GUI objects as arguments to this class so that they
   * can be processed and updated by its methods.
   */
  public void setGUIObjects(
      Vector<ProgressBar> progBars, Vector<ProgressIndicator> progIndicators, TextArea console) {
   ...
   */
  public OptimalParallelBenchmarkAnalyzer(
      String benchRepoPath, String resultsPath, PropertySet properties) {
  ...
  public void analyzeBenchmarkRepo() {
    ...
    while (iterator.hasNext()) {
      ...
      // Add it to the appropriate PropertySet based on the tool that should be used for its
      // quantification
      if (property.getMeasure().getTool().equals(PMDAnalyzer.TOOL_NAME)) {
        pmdPropSet.addProperty(property);
      } else if (property.getMeasure().getTool().equals(CKJMAnalyzer.TOOL_NAME)) {
        ckjmPropSet.addProperty(property);
      } else {
    ...
    try {
      ...
      console.appendText(ex.getMessage() + "\n");
    }
  }

  /**
   * This method is responsible for splitting the work into multiple threads based on the divide and
   * conquer logic.
   *
   * @param pmdPropSet The PMD specific properties
   * @param depth the max depth of divide and conquer
   * @param threshold the max number of threads
   */
  public void pmdThreadCreator(PropertySet pmdPropSet, int depth, int threshold) {
    ...
    List<Property> l1 = pmdPropSet.getProperties().subList(0, half);
    List<Property> l2 = pmdPropSet.getProperties().subList(half, pmdPropSet.size());
                ...
                public void run() {
                  ...
                  for (File project : projects) {
                    ...
                    if (project.isDirectory()) {
                      ...
                      pmd.analyze(
                          project.getAbsolutePath(), resultsPath + "/" + project.getName(), ps1);
                    }
                    progress++;
                  }
              ...
              new Runnable() {
                ...
                public void run() {
                  ...
                  for (File project : projects) {
                    ...
                    if (project.isDirectory()) {
                      pmd.analyze(
                          project.getAbsolutePath(), resultsPath + "/" + project.getName(), ps2);
                    }
                    progress++;
                  }
                  Platform.runLater(
  ...
  /**
   * This method prints the structure of the benchmark directory to the console.
   *
   * <p>It should be used for debugging purposes only.
   */
